...
//                 console.warn("mousedown标记开始移动bDraging="+bDraging);
            ...
            function getPosOffset( ele ) {
                var T = parseInt( ele.css('top') ) || 0,
                    L = parseInt( ele.css('left') ) || 0;
            ...
            function freshPos() {
                var offset = opts.selectEle.offset(),
                    topRaw = offset.top,
                    leftRaw = offset.left,
                    pos = getPosOffset( opts.selectEle );

                dragParams.topRaw = topRaw;
                dragParams.leftRaw = leftRaw;
                dragParams.posX = pos.posX;
                dragParams.posY = pos.posY;
            }
            //移动过程
            focuEle.bind('mousemove',
            function(e) {
                if (bDraging) {
...
//                     console.warn("mousemove标记move移动");
                    //是否限定在某个区域中移动.
...
//                        console.warn("left:"+moveEle.css("left")+",top:"+moveEle.css("top"));
                    } else if (opts.dragDirection == 'vertical') {
                        moveEle.css({
                            'top': dragParams.moveY
                        });
//                        console.warn("top:"+moveEle.css("top"));
                    } else if (opts.dragDirection == 'horizontal') {
                        moveEle.css({
                            'left': dragParams.moveX
                        });
//                        console.warn("left:"+moveEle.css("left"));
                    }
                    //如果有回调
                    if (opts.callback) {
                        //将dragParams作为参数传递
...
//                 console.warn("移动结束bDraging="+bDraging);
            });
        });
    };
    //默认配置
