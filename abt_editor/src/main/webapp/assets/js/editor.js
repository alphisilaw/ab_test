 ...
 *  @brief editor js
 */
function getDefinedStylePropertyByName( node, authorOnly, propname ) {
    var value = null;
    ...
    if( node.style ) {
        ...
        if( node.style.getPropertyPriority(propname) ) {
            return value;
        }
    }

    var styleRules = node.ownerDocument.defaultView.getMatchedCSSRules(node, '', authorOnly);//不能获取不同源的样式，只有chrome支持

    if( !styleRules ) {
        return value;
    }

    for( var i = styleRules.length - 1; i >= 0; i-- ) {//从后往前遍历
        var style = styleRules[i].style;

        if( style.getPropertyPriority( propname ) ) {//是否有important，有的话直接返回
...
$(function () {
    ...
    var dataCtl = (function () {
         ...
         * 保存实验前添加所有kpi指标，业务需要
         */
        var doSaveGoal = function (type, max, callback) {
        	var params = {
        ...
        var showGoals = function (hide) {
            ...
            if (hide) {
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    var $el = editorConf.win['eval'](";(function($){try{ return " + item.goalSelector +
        ...
        var saveDoc = function ($doc) {
            ...
            if (!_f) {
                dataCtl.cacheDoc[editorConf.baseEditFrame.attr('id')] = $doc.find('body').clone();
            }
        ...
        var resetDoc = function (clone, cb) {
            ...
            if (clone) {
                ...
                $body.replaceWith(dataCtl.cacheDoc[curVersion.editFrameId]);
            ...
            editorConf.selector = editorConf.curDoc.find('.medialab-abtest-selector');
            setTimeout(function () {
                cb && cb();
            }, 500);
        }
    ...
    var loadEditPage = function (url) {
        ...
        if (editorConf.caseData.isMobile == 1) {//设置为iphone6
            iframe.css({'width': '375px'});
            // $('#base_editor').css('overflow-y', 'auto');
        }
        iframe.bind('load', onEditPageLoad);
        iframe.attr('src', url);
    }

    var parseEditUrl = function (url) {
    	if (!/^http/.test(url)) {
            url = url.replace(/^\/+/, '', url);
            url = 'http://' + url;
        }
        var match = /(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:\/~\+#]*[\w\-\@?^=%&amp;\/~\+#])?/.exec(url);
        var pre = match[3];
        var spiderdomain = '';
        if (pre == '/') {
        	pre = '';
        	...
        	spiderdomain = url.substring(0,url.indexOf(pre));
        } else {
        	spiderdomain = url;
        }
        $.cookie('spiderdomain', spiderdomain, 'forever');
        $.cookie('isMobile', editorConf.caseData.isMobile, 'forever');
        return SPIDER_URL + (pre || '');
    }
    ...
    var onEditPageLoad = function (e) {
        ...
        /* ADD wap最小高度480px by tongxiaoqin 2016.5.30*/
        if( editorConf.caseData.isMobile == 1 && editorConf.curDoc.height() <= 667 ) {
            iframe.css({'min-height': '667px', 'height': editorConf.curDoc.height()+17});
            $('#base_editor').css('overflow-y', 'auto');
        }
        /*END ADD*/
    ...
    function createQcordeBox( tmpUrl ) {
    ...
    function qcordeDialog( url ) {
        var AddLab = new Dialog({
                html:'<div style="display:none;min-width:300px;" class="dialog">\
                                <div class="dialog_title" id="qcordeTitle">预览链接地址</div>\
    ...
    //发送html获取url
    function postHtmlToGetUrl() {
        var str = encodeURIComponent( editorConf.curDoc.find('body')[0].outerHTML );
        var orenginUrl = $('#_zq-view-mode').find('[t="current-url"]').attr('current-url');
        var isMobile = editorConf.caseData.isMobile;

        $.ajax({
            type: 'post',
            url: '/abt_spier/setVersionHtml',
            data: {url: orenginUrl, str: str, isMobile: isMobile},
            dataType: 'text',
    ...
    var bindEditEvents = function () {
        ...
        }).delegate('[t=review]', 'click', function () {
            postHtmlToGetUrl();
        });
    ...
    function handleSelectors( arr ) {
        var selec = '$(\'',
            flag = false;

        for( var i = 0, len = arr.length; i < len; i++ ) {
            if( typeof arr[i] == 'object' ) {
                if( i - 1 >= 0 && typeof arr[i - 1] != 'object' ) {
                    selec += '\')';
                }
                selec += '.children().eq('+arr[i].idx+')';
                if( i != len - 1 && i + 1 < len && typeof arr[i+1] != 'object' ) {
                    selec += '.find(\'';
                } else {
                    flag = true;
                }
        ...
        selec += typeof arr[len-1] == 'object' ? '' : '\')';
        selec = selec.replace(/>\'/g, "'");
        // console.log( arr, selec );
        return selec;
    }
    ...
    var dropMenu = (function () {
        ...
        var init = function () {
            ...
            $menu.find('.menu_list').html(
                ...
                <li >\
                <div id = "edit-center"   class="menu medit">' +
                '<img src = "' + EDITOR_URL +
                '/assets/images/icon_center.png" / >\
                <span > 居中 </span>\
                </div>\
            ...
            $menu.delegate('.menu', 'click', function () {
                ...
                switch (type) {
                case 'edit-text':
                    var textTagName = $el[0].tagName === 'INPUT';
                    var textVal = $el.text();
                    if( textTagName ) {
                        textVal = $el.val();
                    }
                    popMenu.show('edit-text', '编辑文本', textVal, function (flag, html) {
                        if (flag) {
                            var code = el_slt+'.text("' + html.replace(/\"/gi, '\\"') + '")';
                            if( textTagName ) {
                                code = el_slt+'.val("' + html.replace(/\"/gi, '\\"') + '")';
                            }
                            dataCtl.addCode({
                                slt: el_slt,
                                code: code
                    ...
                    break;
                case 'edit-center':
                    popMenu.show('edit-center', '居中', $el, function (flag, html) {
                        if (flag) {
                            var code = el_slt+'.css(' + html.cssText.replace(/\"/gi, '\\"') + ')';
                            if( html.flag != 'self' ) {
                                code = el_slt+'.parent().css(' + html.cssText.replace(/\"/gi, '\\"') + ')';
                            }
                            dataCtl.addCode({
                                slt: el_slt,
                                code: code
                ...
                case 'edit-sty':
                    ...
                    break;
                case 'edit-move':
                    var topCSS = getDefinedStylePropertyByName($el[0], false, 'top'),
                        leftCSS = getDefinedStylePropertyByName($el[0], false, 'left'),
                        rightCSS = getDefinedStylePropertyByName($el[0], false, 'right'),
                        bottomCSS = getDefinedStylePropertyByName($el[0], false, 'bottom');

                    // editorConf.caseData.isMobile
                    if( checkRelativeUnit(topCSS) || checkRelativeUnit(leftCSS) || checkRelativeUnit(rightCSS) || checkRelativeUnit(bottomCSS)) {
                        if( !confirm('此元素偏移为相对大小，移动位置后无法适配所有的屏幕尺寸，确定要继续吗？') ) {
                            return;
                        }
                    }
                    popMenu.show('edit-move', '移动', $el, function (flag, html) {
                        if (flag) {
                            var code = el_slt+'.css({\'position\':\'relative\',\'top\':\'' + html.top + '\',\'left\':\'' + html.left +
                                '\'})';
                            dataCtl.addCode({
                                slt: el_slt,
                                code: code
                    ...
                    break;
                case 'edit-drag':
                    var widthCSS = getDefinedStylePropertyByName($el[0], false, 'width'),
                        heightCSS = getDefinedStylePropertyByName($el[0], false, 'height');

                    if( checkRelativeUnit(widthCSS) || checkRelativeUnit(heightCSS) ) {
                        if( !confirm('此元素大小为相对大小，调整大小无法适配所有的屏幕尺寸，确定要继续吗？') ) {
                            return;
                        }
                        $('.medialab-abtest-selector').addClass('medialab-abtest-selector-on');
                    }
            ...
            function checkRelativeUnit( unit ) {
                if( unit == 0 || unit.indexOf('px') !== -1 ) {
                    return false;
                }
    ...
    var popMenu = (function () {
        ...
        var show = function (type, title, content, cb, onChange) {
            ...
            function setValue(content) {
                ...
                case 'edit-htm':
                    ...
                    break;
                case 'edit-img':
                    var src = $popDom.find('input:text');
                    src.val(content);
                    break;
                case 'edit-url':
                ...
                case 'edit-sty':
                    for (var i in styles) {
                        ...
                        if (styles[i] == 'color' || styles[i] == 'background-color') {
                            $popDom.find('input:text.sty-' + styles[i]).val(styleTmp).attr('data-orgval', styleTmp);
                            ...
                            // }
                            $popDom.find('input.sty-' + styles[i]).val(styleTmp).attr('data-orgval', styleTmp);
                        }
                    }
                    break;
                case 'edit-common':
            ...
            function getCssProp( ele ) {
                var display = ele.css('display'),
                    position = ele.css('position'),
                    flag = 'self',
                    cssText = '';

                var widthCss = getDefinedStylePropertyByName(ele[0], false, 'width');
                var MLCss = getDefinedStylePropertyByName(ele[0], false, 'margin-left');
                var MRCss = getDefinedStylePropertyByName(ele[0], false, 'margin-right');

                if( !widthCss ) {
                    widthCss = ele.width().toString();
                }
                var unit = 'px';

                if( widthCss.indexOf('%') != -1 ) {
                    unit = '%';
                } else if( widthCss.indexOf('rem') != -1 ) {
                    unit = 'rem';
                } else if( widthCss.indexOf('em') != -1 ) {
                    unit = 'em';
                }
                widthCss = widthCss.replace(/[^\d\.]/g, '');
                var w = -widthCss/2;

                if( position == 'static' ) {
                    cssText = '{\'display\': \'block\',\'margin-left\': \'auto\', \'margin-right\': \'auto\'}';
                } else {
                    cssText = '{\'left\': \'50%\', \'margin-left\': \''+w+unit+'\'}';
                }
            ...
            function getValue() {
                ...
                case 'edit-move':
                    ...
                    return {
                        top: currentContent.css('top'),
                        left: currentContent.css('left')
                    }
                    break;
                case 'edit-drag':
                    // return {
                    ...
                    // };
                    // var dragText = '{\'width\': '+currentContent.outerWidth()+', \'height\': '+currentContent.outerHeight();
                    // if( currentContent.css('display') == 'inline' ) {
                    //     dragText += ',\'display\': \'inline-block\'';
                    // }
                    // dragText += '}';
                    // return dragText;
                    return {
                ...
                case 'edit-sty':
                    ...
                    for (var i in styles) {
                        var styleTmp = $popDom.find('input:text.sty-' + styles[i]).val();

                        if( styleTmp != $popDom.find('input:text.sty-' + styles[i]).attr('data-orgval')) {
                            changedFlag = true;
                            if (styles[i] == 'color' || styles[i] == 'background-color') {
        ...
        var init = function () {
            $popDom.delegate('.btn-ok', 'click', function () {
                ...
                $popDom.undelegate('.upload-img input', 'change');
                ...
                $popDom.hide();
                return false;
            }).delegate('.upload-img input', 'change', function( e ) {
                uploadImg( e );
            });
        }
    ...
    function uploadImg( e ) {
        ...
        xhr.open('post', '/abt_manager/uploadImg', true);
        xhr.onload = function() {
            var url = xhr.responseText;
            $(target).parents('.edit-ads').find('input:text').val( url );
            removeLoading();
        };
        addLoading();
        xhr.send( form );
    ...
    function addLoading() {
        var str = '<div class="up-loading"><div class="load-container load8"><div class="loader">Loading...</div></div></div>';
        $('body').append( str );
    }
    ...
    var goalEditor = (function () {
        ...
        var init = function () {
            $popDom.delegate('.cancel_btn', 'click', function () {
                ...
                var version = dataCtl.getCurVersion();
                var goalLen = editorConf.win['eval'](";(function($){try{ return " + selector +
                    ".length;}catch(e){throw e;}})(window.__medialab_abtest_jQuery)");
                // console.log( goalLen );
                if (version.type == 3) {
                    if (goalLen == 0) {
            ...
            if (self.isload) {
                ...
                var iframeHandler = function () {
                    ...
                    insertEditCode(editorDoc, function ($doc) {
                        ...
                        $doc.find('body').find('script').remove();
                        dataCtl.cacheDoc[iframe.attr('id')] = $doc.find('body').clone();
                    });
                    editorConf.curDoc = editorDoc;
                    if (shallow) {
